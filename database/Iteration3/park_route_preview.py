...
def route_popup(row: Any) -> str:
    return popup_table([
        ...
        ("Task difficulty adjustment", row.get("task_difficulty_adjustment")),
        ...
        ("Task cluster score units", row.get("task_cluster_score_units")),
        ("Backtrack m", row.get("backtrack_m")),
        ("Backtrack ratio", row.get("backtrack_ratio")),
        ("Scored task clusters <=50m", row.get("nearby_task_cluster_count")),
        ("Scored tasks", row.get("nearby_task_count")),
        ("Cluster distances", row.get("nearby_task_cluster_details")),
        ("Start gate", row.get("start_gate_name")),
        ("End gate", row.get("end_gate_name")),
        ("Candidate type", row.get("candidate_type")),
        ("Overlap ratio", row.get("overlap_ratio")),
    ])
